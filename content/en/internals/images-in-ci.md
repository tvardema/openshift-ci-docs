...
[QCI](/how-tos/use-registries-in-build-farm/#summary-of-available-registries),
...
- [promoted](/architecture/ci-operator/#publishing-container-images) by CI jobs, or
- [mirrored from external repositories](/how-tos/external-images/).
...
For example, the above `base` image will lead to a tag specified in [the `pipeline` image stream](/architecture/ci-operator/#referencing-images):
...
However, the additional layers of components on top of `QCI` might increase the amount of challenges for [troubleshooting](/internals/images-in-ci/#troubleshooting) issues related to CI images.
