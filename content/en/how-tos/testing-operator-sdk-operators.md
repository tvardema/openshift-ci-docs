...
[dependencies](/architecture/ci-operator/#referring-to-images-in-tests) feature.
...
The `$OO_BUNDLE_INIT` image in the above example is resolved by [dependency](/architecture/ci-operator/#referring-to-images-in-tests) of the test which is declared as a base image.
It is usually a production image released by the operator in an external registry and then [imported into the CI registry](/how-tos/external-images/).
...
the current [dev branch of OpenShift](/architecture/branching/#branching-phases) and a cluster [hosted by HyperShift](/architecture/ci-operator/#testing-with-a-cluster-from-hypershift).
