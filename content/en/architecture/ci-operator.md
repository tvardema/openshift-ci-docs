...
Note that `cluster_claim` and `cluster_profile` are mutually exclusive because [the latter](/architecture/step-registry/#implicit-lease-configuration-with-cluster_profile) indicates installing a cluster on demand, instead of claiming a pre-installed cluster in a pool.
...
Learn more about multi-stage tests at the [overview.](/architecture/step-registry/)
...
* `reporter_config` Configure [Slack reporting](/how-tos/notification/) for this test with `channel`, `job_states_to_report`, `report_template`, and `report_presubmit`.
...
| `slack_reporter` (pattern-based) | `tests[].reporter_config` (per test, see [Slack notifications](/how-tos/notification/)) |
...
For further information also check [Interacting With CI Image Registries](/how-tos/use-registries-in-build-farm).
