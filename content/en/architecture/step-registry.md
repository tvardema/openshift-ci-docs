...
* [a pipeline image](/architecture/ci-operator/#referencing-images)
* [an external image](/architecture/ci-operator/#configuring-inputs)
* [an image built by `ci-operator`](/architecture/ci-operator/#building-artifacts)
* [an image imported from a release ImageStream](/architecture/ci-operator/#referencing-images)
...
|`${RELEASE_IMAGE_INITIAL}`|Image pull specification for the initial release payload snapshot when the test began to run.|When the test imports or builds an `initial` release. See the [docs](/architecture/ci-operator/#describing-inclusion-in-an-openshift-release).|
|`${RELEASE_IMAGE_LATEST}`|Image pull specification for the ephemeral release payload used to install the ephemeral OpenShift cluster.|When the test imports or builds a `latest` release. . See the [docs](/architecture/ci-operator/#describing-inclusion-in-an-openshift-release).|
...
|`${IMAGE_FORMAT}`|The registry location from which images built or imported for this test may be pulled.|Always except [claiming a cluster](/architecture/ci-operator/#testing-with-a-cluster-from-a-cluster-pool). Deprecated, use [dependencies](/architecture/ci-operator/#referencing-images) to provide tests with fully resolved pull specifications of images.|
...
[this page](/architecture/quota-and-leases)) in two different ways:
...
[dependencies](/architecture/ci-operator#referring-to-images-in-tests), and
[leases](#leases).
...
[cluster profile]({{< ref "/how-tos/adding-a-cluster-profile" >}})
...
[documentation]({{< ref "/how-tos/adding-a-cluster-profile#vpn-connection" >}}).
